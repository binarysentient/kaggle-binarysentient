...
#                 print(book_df)
        ...
        for groupkey, groupdf in trade_df.groupby('time_id'):
            ...
            if rowid not in feature_map:
                feature_map[rowid] = {}
            grouped_interval_df = generate_interval_group_features(groupdf[['price','size','order_count','trade_money_turnover','trade_money_turnover_per_order','seconds_in_bucket_xs_groupkey']], 'seconds_in_bucket_xs_groupkey', intervals_count, 
                                             function={'price':'mean','size':'sum','order_count':'sum', 'trade_money_turnover':'sum','trade_money_turnover_per_order':'sum'}, column_fill_map={'size':'zero','order_count':'zero','trade_money_turnover':'zero','trade_money_turnover_per_order':'zero'})
...
#             feature_map[rowid]['book_realized_volatility'] = generate_realized_volatility_df(groupdf)
            grouped_interval_df = generate_interval_group_features(groupdf[['wap1','wap2','wap_1n2', 'directional_volume1', 'directional_volume2', 'price_spread1', 'price_spread2','bid_spread','ask_spread', 'total_volume', 'volume_imbalance','book_money_turnover1','seconds_in_bucket_xs_groupkey']], 'seconds_in_bucket_xs_groupkey', intervals_count, 
                                             function={'wap1':'mean','wap2':'mean','wap_1n2':'mean', 'directional_volume1':'mean', 'directional_volume2':'mean', 
                                                       'price_spread1':'mean', 'price_spread2':'mean','bid_spread':'mean','ask_spread':'mean', 'total_volume':'sum', 'volume_imbalance':'mean','book_money_turnover1':'sum'},
                                            column_fill_map={'directional_volume1':'zero','directional_volume2':'zero', 'total_volume':'zero', 'volume_imbalance':'zero','book_money_turnover1':'zero'})
